...
% SimSineSim
% The simulator class that defines the SineSim Simulator
classdef SimSineSim < SimNoModelMLOnly
    properties
        version;
    end
    ...
    methods
        ...
        % -------------
        % We override the Simulator.m method in order to demo the Pre Run
        % Upload Simulation Data Files Stage. This can be done in any user
        % simulator.
        function [sourceFileList, destFileList] =...
                    defineSimulationInputDataFiles(obj, simulation)  %#ok<INUSD>
            % User can also get these strings from the simulation's
            % input parameter list, which is where the power lies.  See
            % Sim_Neur_Purkinje_Miyasho2001_Kh.m for an example of doing
            % that. 
            sourceFileList = {'SampleInputDataFile.txt'}; 
            destFileList = {'RenamedSampleInputDataFile.txt'};
        end
        
        % -------------
        function postDownloadProcessingSimulatorSpecific(obj, simulation)  %#ok<INUSL>
            % Copy the QuickSee file for auto attachment to the last
            % notification for the supplied simulation.
            sourceFile = fullfile(simulation.getHostBaseDir(), 'QuickSee.pdf');
            if exist(sourceFile, 'file')
                destFile = fullfile(simulation.getHostBaseDir(),...
