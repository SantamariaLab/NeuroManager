...
% Configuration: Linux host with heterogenous Virtual Machine; option to
% run remotely by logging in to host machine via SSH (phone, tablet,
% laptop, or desktop). More detailed explanation in User Guide.  
...
if usejava('Desktop')
    % This case for operation directly from the MATLAB GUI
    nm = NeuroManager(nmDirectorySet, nmAuthData, userData,...
                      'logEchoFlag', true, 'useDualKey', true);
    disp('Using Visual MATLAB Gui settings');
else 
    % This case allows different settings for remote operation of
    % NeuroManager without MATLAB GUI (see User Guide); the log echo allows
    % seeing status remotely in a serial way. The remote version will not
    % show the in-GUI webpage but most of its information is contained in
    % the log stream.
    nm = NeuroManager(nmDirectorySet, nmAuthData, userData,...
                      'logEchoFlag', true, 'useDualKey', true);
    disp('Using MATLAB without Gui');
end
...
% Shut down the MATLAB host instance if remote
if ~usejava('Desktop')
	exit;
end
