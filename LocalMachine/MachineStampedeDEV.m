...
% MachineStampedeDEV
% Specifies the dev queue on Stampede.
classdef MachineStampedeDEV < MachineStampede
    methods
        function obj = MachineStampedeDEV(wallClockTime,...
                            hostID, hostOS, baseDir,...
                            scratchDir, simFileSourceDir,...
                            custFileSourceDir, modelFileSourceDir,...
                            simType, inNumSims,...
                            auth, log, notificationSet) 
            qd = StampedeQueueData.DEV;
            numSims = 1; % TACC Development queue limit is one job at a time
            
            obj = obj@MachineStampede(qd, wallClockTime,...
                            ...
                            auth, log, notificationSet);
            if inNumSims > 1
                obj.log.write(['Number of simulators for StampedeDEV'...
                               ' set to 1 due to queue limit.']);
            end
        end
    end
end
