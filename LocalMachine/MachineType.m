...
% MachineType
% A class which defines the type a Machine can be and associates type with
% the constructors required to work with that type.
classdef MachineType
    properties
        constrFunc;         % Machine constructor
        commsTestFunc;      % Test machine constructor
    end
    methods
        function t = MachineType(constrFunc, commsTestFunc)
            t.constrFunc = constrFunc;          
            t.commsTestFunc = commsTestFunc;    
        end
    end
    enumeration
        UNASSIGNED          (0, 0)
        UNKNOWN             (0, 0)
        %--
        MYSERVER01          (@MachineMyServer01,...
                             @MachineMyServer01Test)
        %--
        MYSERVER02          (@MachineMyServer02,...
                             @MachineMyServer02Test)
        %--                                    
        MYSGECLUS01ALL      (@MachineMySGEClus01ALL,...
                             @MachineMySGECluster01Test)
                         
        MYSGECLUS01BM       (@MachineMySGEClus01BM,...
                             @MachineMySGECluster01Test)
                         
        MYSGECLUS01GPU      (@MachineMySGEClus01GPU,...
                             @MachineMySGECluster01Test)
                         
        MYSGECLUS01IB       (@MachineMySGEClus01IB,...
                             @MachineMySGECluster01Test)
        %--
        STAMPEDEDEV         (@MachineStampedeDEV,...
                             @MachineStampedeClusterTest)
                         
        STAMPEDENORMAL      (@MachineStampedeNORMAL,...
                             @MachineStampedeClusterTest)
    end
end 
