...
% StampedeQueueData.m
% A class which identifies the queues on the Stampede Cluster at TACC and
% associates them with their extension and jobstring.
classdef StampedeQueueData
    properties
        extension;
        jobString;
    end
    methods
        function qd = StampedeQueueData(extension, jobString)
            qd.extension = extension;
            qd.jobString = jobString;
        end
    end
    enumeration
        Unknown ('UNK',     '')
        ...
        DEV     ('DEV',     'development')
        GPU     ('GPU',     'gpu')
        GPUDEV  ('GPUDEV',  'gpudev')
        LGMEM   ('LM',      'largemem')
        NORMMIC ('NM',      'normal-mic')
    end
end
