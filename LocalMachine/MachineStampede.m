...
% MachineStampede
classdef MachineStampede < MachineSLURMCluster & NeuronMachine
    methods
        function obj = MachineStampede(qd, wallClockTime,...
                            hostID, hostOS, baseDir, scratchDir,...
                            simFileSourceDir, custFileSourceDir,...
                            modelFileSourceDir,...
                            simType, numSims,...
                            auth, log, notificationSet) 
            md = createStampedeClusterData();
            obj = obj@NeuronMachine(md);
            numNodes = 1; % For future investigation
            
            % MATLAB Compilation for TACC is done on another machine.
            % For now we assume Server 02 since it matches MATLAB version
            % with TACC, but SGECluster01 also matches and works.
            if 0
                xCompilationMachine =...
                            XCompileMyServer02(hostID, hostOS,...
                                               'COMPILE', auth); %#ok<UNRCH>
                mdx = createMyServer02Data();
                xCompilationScratchDir =...
                            mdx.getSetting('xCompDir');
            else
                xCompilationMachine =...
                            XCompileMySGECluster01(hostID, hostOS,...
                                                   'COMPILE', auth); 
                mdx = createMySGECluster01Data();
                xCompilationScratchDir =...
                            mdx.getSetting('xCompDir');
            end

            obj = obj@MachineSLURMCluster(md, qd,...
                            numNodes, wallClockTime,...
                            hostID, hostOS, baseDir,...
                            scratchDir, simFileSourceDir,...
                            custFileSourceDir, modelFileSourceDir,...
                            simType, numSims,...
                            xCompilationMachine,...
                            xCompilationScratchDir,...
                            auth, log, notificationSet);
        ...
        % Part of PreRunModelProcPhaseP; refer to NeuroManagerStaging.xlsx
        ...
        % ----------
        % Neuron model processing is machine-dependent
        % Part of PreRunModelProcPhaseD; refer to NeuroManagerStaging.xlsx
        function str = getCompileNeuronModelFilesStrPhaseD(obj, simulation)%#ok<INUSD>
            str = '';  % Nothing to do here on Stampede
        end
    end
     
	% May have to override GetJobID() here
