...
function md = createStampedeClusterData()
    userName = 'usernamehere';
    ipAddress = '00.00.00.00';
    % An empty string password means use dual key authentication
    password = '';

    md = RMD();
    md.addSetting('id',                  'StampedeCluster');
    ...
    md.addSetting('matlabCompilerDir',   '/work/apps/matlab/2013a/bin');
    md.addSetting('matlabCompiler',      'mcc');
    md.addSetting('matlabExecutable',    '');  % No license available on Stampede
    md.addSetting('xCompDir',            '/work/xxxxx/username/SMComp');
    md.addSetting('mcrDir',              '/work/xxxxx/username/MATLAB/MATLAB_Compiler_Runtime/v81');

    md.addSetting('neuronDir',           '/home1/xxxxx/username/neuron');
    ...
    md.addSetting('neuronPythonLibExt',  '/nrn/lib/python2.7');
    md.addSetting('neuronCompModLibExt', '/x86_64/.libs');
    md.addSetting('envAddlLibLines',     '');
    md.addSetting('pythonPath',          '/opt/apps/python/epd/7.3.2/bin/python');
    % A cell array of lines to add to the environment setup for Neuron
    % Python simulators. 
	md.addSetting('pythonEnvAddlLines',  {'module load python';...
                                          ['export PYTHONPATH='...
                                           md.getSetting('neuronDir')...
                                           md.getSetting('neuronPythonLibExt')...
                                                    ':$PYTHONPATH']});
    % A cell array of lines to add to the environment setup for non-Python
    % Neuron simulators. 
    % Something about our Neuron installation requires Python environment
    % variables even though we are not using the -python setting.
    md.addSetting('envAddlLines',        md.getSetting('pythonEnvAddlLines'));
end 
