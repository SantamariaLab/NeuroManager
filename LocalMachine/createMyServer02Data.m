...
% createMyServer02Data
% Loads up an RMD with myServer02's information
function md = createMyServer02Data()
    userName = '';
    ipAddress = '';
    ...
    md.addSetting('matlabCompilerDir',   '/usr/local/bin');
    md.addSetting('matlabCompiler',      'mcc.matlab');
    md.addSetting('matlabExecutable',    'matlab');
    md.addSetting('mcrDir',              '/usr/local/MATLAB/MATLAB_Compiler_Runtime/v81');

    % This setting only for cross-compiling on this machine
    md.addSetting('xCompDir',            '/home/username/SMComp'); %<--------

    % Neuron settings for this machine are set below (these are example
    % directories)
    md.addSetting('neuronDir',           '/opt/neuron');
    md.addSetting('neuronHomeExt',       '/share/nrn');
    md.addSetting('neuronBinExt',        '/x86_64/bin');
    md.addSetting('neuronLibExt',        '/x86_64/lib');
