...
% createMyServer01Data
% Sets specific data for myServer01.
function md = createMyServer01Data()
    userName = 'usernamehere';
    ipAddress = '00.00.00.00';
    % An empty string password means use dual key authentication unless
    % using single machine configuration
    password = '';
    
    md = RMD();
    md.addSetting('id',                  'machinenamehere');

    % For this server we assume the fs and js machines are the same
    % machine, so these are set above
    ...
    % These are for the MATLAB software on this machine.
    md.addSetting('matlabCompilerDir',   '/usr/local/MATLAB/R2012b/bin');
    md.addSetting('matlabCompiler',      'mcc');
    md.addSetting('matlabExecutable',    'matlab');
    md.addSetting('mcrDir',              '/usr/local/MATLAB/MATLAB_Compiler_Runtime/v80');

    % This setting only for cross-compiling by another machine using this machine
    md.addSetting('xCompDir',            '/home/username/SMComp'); % <---- change this if necessary

    % Neuron settings for this machine are set below (these are example
    % directories)
    md.addSetting('neuronDir',           '/opt/neuron73');
    md.addSetting('neuronHomeExt',       '/nrn/share/nrn');
    md.addSetting('neuronBinExt',        '/nrn/x86_64/bin');
    md.addSetting('neuronLibExt',        '/nrn/x86_64/lib');
    md.addSetting('neuronPythonLibExt',  '');
    md.addSetting('neuronCompModLibExt', '/x86_64/.libs');
    md.addSetting('envAddlLibLines',     '');
    md.addSetting('pythonPath',          '/usr/bin/python');
    % A cell array of lines to add to the environment setup for Neuron
    % Python simulators if necessary. 
    md.addSetting('pythonEnvAddlLines',  {});
    % A cell array of lines to add to the environment setup for non-Python
    % Neuron simulators if necessary. 
    md.addSetting('envAddlLines',        {});
end 
