...
function [result, errmsg] =...
    ...
    % We don't use the leak value here; it has already been incorporated
    % into the on-the-fly Leak.mod created on the host.  However, we do
    % use it in visualization later in this file.
    leakStr = varargin{3}; 
    dataFile = [simID '.dat'];
    dataFilePath = fullfile(runDir, dataFile);

    % In Example02C we create our parameters.hoc on the fly with input
    % values; also name our data file using the simid for better provenance.
    parameterFile = fullfile(runDir, 'parameters.hoc');
    pFile = fopen(parameterFile, 'w');
    fprintf(pFile, '%s\n', ['// parameters.hoc for ' simID]); 
    fprintf(pFile, '%s\n', '// This file generated automatically; do not edit.'); 
    fprintf(pFile, '%s\n', 'print "parameters: defining variables"'); 

    % Current and duration values are passed straight
    % through, but could have range/validity checking here if desired.
    fprintf(pFile, 'icurrent = %s\n', currentStr); 
    fprintf(pFile, 'idelay = %s\n', '100'); 
    fprintf(pFile, 'iduration = %s\n', durationStr); 
    fprintf(pFile, 'tstop = %s\n', '300'); 
    fprintf(pFile, 'dt = %s\n', '0.010'); 
    fprintf(pFile, 'dt2 = %s\n', '0.020'); 
    fprintf(pFile, 'v_init = %s\n', '-65'); 
    fprintf(pFile, 'celsius = %s\n', '37'); 
    % We are relying on the default value of gl_Leak so it does not go into
    % this file.  In manipulating the mod file we are manipulating its
    % default value.
    fprintf(pFile, '%s\n', 'strdef fname');
    fprintf(pFile, 'fname = "%s"\n', dataFile);
    fclose(pFile);
    copyfile(parameterFile, outputDir); % for documentation
    ...
    if status == 0 % i.e, successful return from the simulation
        if (~exist(dataFilePath, 'file'))
            errmsg = ['Could not find data file ' dataFilePath '.'];
            result = 1;
            return;
        else
            % Download the data 
            copyfile(dataFilePath, outputDir);
        end
        
        % A little visualization
        load(dataFilePath);
        % Figure visibility doesn't seem to matter as far as remote operation
        % is concerned, and since it is post-simulation it doesn't really
        % affect performance. So we leave it visible to make opening
        % simpler on the host (if saved as not visible, have to open from
        ...
        title({['SimpleSpike02C'];...
               ['SimID = ' simID];...
               ['icurrent = ' currentStr ' nA; Leak\_gl = ' leakStr...
                '; duration = ' durationStr]});
        xlabel('Time (msec)');
        ylabel('Voltage (mV)');
        grid on;
        filename = [simID '_current_' leakStr '_leak_gl_'...
                    currentStr '.fig'];
        ...
        errmsg = ['UserSimulation.m received nonzero (' num2str(status)...
                  ') status from the RunHocOnlySimulation call.'];
        % Will result in a failed simulation but NeuroManager will continue
