...
function [result, errMsg] =...
    ...
    % class files.  However, we set them up here for use in plotting.
    mAlphaAstr = varargin{1};
    mAlphakstr = varargin{2}; 
    mBetaAstr =  varargin{3}; 
    mBetakstr =  varargin{4}; 
    hAlphaAstr = varargin{5}; 
    hAlphakstr = varargin{6}; 
    hBetaAstr =  varargin{7}; 
    hBetakstr =  varargin{8};
    
    dataFile = [simID '.dat'];
    dataFilePath = fullfile(runDir, dataFile);

    % In Example03 we create our parameters.hoc on the fly with static
    ...
    % Current and duration values are set here for all simulations in this simulator.
    ...
    if status == 0 % i.e, successful return from the simulation
        ...
        title({['SimpleSpike03'];...
               ...
               ['hAlphaA = ' hAlphaAstr ', '...
                'hAlphak = ' hAlphakstr ', '...
                'hBetaA = '  hBetaAstr  ', '...
                'hBetak = '  hBetakstr]}, 'Interpreter', 'none');
        xlabel('Time (msec)');
        ylabel('Voltage (mV)');
        grid on;
        filename = [simID '.fig'];
