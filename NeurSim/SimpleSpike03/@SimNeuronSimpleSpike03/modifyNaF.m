...
% modifyNaF
% Takes an input NaF NeuroML XML file, edits zero or more of twelve
% parameters, and creates a new output file with the changes, leaving the
% original untouched.  Does not validate the input or output files.
% As is natural, this function depends highly on the NaF file structure.
% This function does not make use of the ModifyNaF default values.
% infile and outfile are required inputs and should be full absolute
% pathnames. 
% Parameters that this function modifies are:
% mAlphaA, mAlphak, mAlphad, mBetaA, mBetak, mBetad,
% hAlphaA, hAlphak, hAlphad, hBetaA, hBetak, hBetad.
% Only parameters that are explicitly listed in the function call will be
% modified.
function modifyNaF(obj, varargin)
               
 	p = obj.modifyNaFInputParser();
    parse(p, varargin{:});
    ...
    % Validate the input file here
    % (not implemented)
    
    % Pull in the file for surgery
    try
       myDOM = xmlread(inFile);
    catch
       error('Failed to read XML file %s.', inFile);
    end
    ...
    try
        ...
        error('Could not create modified NaF file %s.', outFile);
    end
   
    % Files ok, now set up XPath
    import javax.xml.xpath.*
    factory = XPathFactory.newInstance;
    xpath = factory.newXPath;

    xRoot = myDOM.getDocumentElement;

    % Add NeuroManager to the list of Model Translators
    expression = ...
        xpath.compile(['//*[name()="channel_type"]'...
                       '/*[name()="meta:authorList"]']);
    authNode = expression.evaluate(xRoot, XPathConstants.NODE);

    mTnode = myDOM.createElement('modelTranslator');
    mTname = myDOM.createElement('meta:name');
    mTtext = myDOM.createTextNode('NeuroManager');
    mTname.appendChild(mTtext);
    mTnode.appendChild(mTname);
    authNode.appendChild(mTnode);

    CommentNode = myDOM.createElement('meta:comment');
    mTtext = myDOM.createTextNode(['Modified for Simulation ' p.Results.simid]);
    CommentNode.appendChild(mTtext);
    mTnode.appendChild(CommentNode);

    
    % Modify any supported variable that is not in the call's input
    % parameters; the remainder are unmodified. In addition every modified
    % value gets logged in the metadata for provenance.

    % mAlphaA
    if ~any(strcmp('mAlphaA', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
                           '/*[name()="voltage_gate"]'...
                           '/*[name()="alpha"]'...
                           '/*[name()="parameterised_hh"]'...
                           '/*[name()="parameter" and @name="A"]'...
                           ]);
        editValue(expression, 'mAlphaA');
    end
    
    % mAlphak
    if ~any(strcmp('mAlphak', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
                           ...
                           '/*[name()="alpha"]'...
                           '/*[name()="parameterised_hh"]'...
                           '/*[name()="parameter" and @name="k"]'...
                           ]);
        editValue(expression, 'mAlphak');
    end
    
    % mAlphad
    if ~any(strcmp('mAlphad', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
                           ...
                           '/*[name()="alpha"]'...
                           '/*[name()="parameterised_hh"]'...
                           '/*[name()="parameter" and @name="d"]'...
                           ]);
        editValue(expression, 'mAlphad');
    end
    
    % mBetaA
    if ~any(strcmp('mBetaA', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'mBetaA');
    end
    
    % mBetak
    if ~any(strcmp('mBetak', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'mBetak');
    end
    
    % mBetad
    if ~any(strcmp('mBetad', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="m"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'mBetad');
    end
    
    % -------------------
    
    % hAlphaA
    if ~any(strcmp('hAlphaA', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
                           '/*[name()="voltage_gate"]'...
                           '/*[name()="alpha"]'...
                           '/*[name()="parameterised_hh"]'...
        ...
        editValue(expression, 'hAlphaA');
    end
    
    % hAlphak
    if ~any(strcmp('hAlphak', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'hAlphak');
    end
    
    % hAlphad
    if ~any(strcmp('hAlphad', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'hAlphad');
    end
    
    % hBetaA
    if ~any(strcmp('hBetaA', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'hBetaA');
    end
    
    % hBetak
    if ~any(strcmp('hBetak', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'hBetak');
    end
    
    % hBetad
    if ~any(strcmp('hBetad', p.UsingDefaults))
        expression = ...
            xpath.compile(['//*[name()="channel_type"]'...
                           '/*[name()="hh_gate" and @state="h"]'...
                           '/*[name()="transition"]'...
        ...
        editValue(expression, 'hBetad');
    end
    
    % -------------------
    % Write it all out to file
    xmlwrite(outFile, xRoot)
    ...
    % -------------------
    % Nested functions to simplify above code
    function editValue(expression, varname)
        import javax.xml.xpath.XPathConstants;     % For the XPathConstants
        kNode = expression.evaluate(xRoot, XPathConstants.NODE);
        expression = xpath.compile('./@value');
        oldValue = expression.evaluate(kNode, XPathConstants.STRING);
        newValue = p.Results.(varname);
        kNode.setAttribute('value', newValue);

        % Add the change to the metadata
        addMetaComment(['Changed ' varname ' from ' oldValue ' to ' newValue]);
    end

    function addMetaComment(comment)
            CommentNode = myDOM.createElement('meta:comment');
            mTtext = myDOM.createTextNode(comment);
            CommentNode.appendChild(mTtext);
            mTnode.appendChild(CommentNode);
    end
end
