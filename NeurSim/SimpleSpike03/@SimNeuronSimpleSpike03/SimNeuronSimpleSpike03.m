...
% SimNeuronSimpleSpike03
% A hoc-only Neuron simulator that changes three input parameters by
% constructing both parameters.hoc and a mod file on-the-fly in the
% target-side UserSimulation.m file. 
classdef SimNeuronSimpleSpike03 < SimNeuron
    properties
        version;
        
        % Your transformation stuff here
        % Stylesheet from https://neuroml.org/neuron_tools
        % They use 4suite library with python instead; we could use that in
        % the Python port.
        % xslt 2.0 processor with xslt 1.0 stylesheet.
        % The Warning has been suppressed!!!
        xmlTransformLoc = 'C:/Saxon';
    ...
    methods
        function obj = SimNeuronSimpleSpike03(id, machine, type, dbH, ...
                                                    log, notificationSet)
            ...
            % to construct it or modify one of these on-the-fly.
            % Trim this as much as possible to reduce upload and
            % compile time.
        ...
        % directory. Abstract is in Sim_Neuron.
            ...
            % scratch dir but don't want one on the target. 
            % In contrast to 02C we are not building the entire file here.
            % Instead, we are making a modified copy of the NeuroML version
            % using a custom function that has been added to this class
            % (see below), then we are using the stylesheet to transform
            % the XML file into a mod file, then uploading the mod file.
            
            % Need to add existence checks
            originalNMLFilename = 'NaF_Chan.xml';
            originalNMLFile = fullfile(obj.machine.getModelFileSourceDir(),...
                originalNMLFilename);
            
            hostNMLFilename = [simulation.getID(), 'NaF_ChanMOD.xml'];
            nmlFile = fullfile(obj.machine.getScratchDir(),...
                               hostNMLFilename);
                           
            hostModFilename = [simulation.getID(), 'NaF.mod'];
            hostModFile = fullfile(obj.machine.getScratchDir(),...
                               hostModFilename);
                           
            % We rename the file when it gets to the target so that the
            % simulation's expectations are met.
            targetModFilename = 'NaF.mod';

            % Modify the file according to input parameters
            obj.modifyNaF(obj,...
                          originalNMLFile, nmlFile,...
                          simulation.getID(),...
                          'mAlphaA', simulation.getParam(1), ...
                          'mAlphak', simulation.getParam(2), ...
                          'mBetaA',  simulation.getParam(3), ...
                          'mBetak',  simulation.getParam(4), ...
                          'hAlphaA', simulation.getParam(5), ...
                          'hAlphak', simulation.getParam(6), ...
                          'hBetaA',  simulation.getParam(7), ...
                          'hBetak',  simulation.getParam(8));
            
            % Transform it using stylesheet into a mod file
            styleSheetFile = fullfile(obj.machine.getModelFileSourceDir(), ...
                obj.xml2MODStylesheet);
            transformString = ['java -jar ' ...
                fullfile(obj.xmlTransformLoc, obj.xmlTransformFile) ...
                ' -s:' nmlFile ...
                ' -xsl:' styleSheetFile ...
                ' -o:' hostModFile ...
                ' -versionmsg:off'];
            ...
            % Copy these simulation-specific files to the results dir for
            % provenance.
            copyfile(nmlFile, simulation.getHostBaseDir());
            copyfile(hostModFile, simulation.getHostBaseDir());
    ...
    methods(Static)
        modifyNaF(p, varargin)
        p = modifyNaFInputParser()
        result = filecheck(x)
        result = dircheck(x)
    end
end
