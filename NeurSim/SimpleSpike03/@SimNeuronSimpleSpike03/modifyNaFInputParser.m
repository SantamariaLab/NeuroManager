...
% modifyNaFInputParser
% Note: dual verification and defaults 
function p = modifyNaFInputParser()
    p = inputParser;
    ...
    addRequired(p, 'infile', @filecheck);
    
    % Add-required output file
    addRequired(p, 'outfile', @dircheck);
    
    % Add-required simid
    addRequired(p, 'simid', @ischar);
    
    % Beyond scope of this function to do a seriously rigorous input number
    % check; Reference Walter Robinson entry at
    % http://www.mathworks.com/matlabcentral/answers/
    %    3828-how-to-determine-if-a-string-can-be-converted-
    %    to-a-numerical-value
    numbercheck = @(x) all(ismember(x, '0123456789+-.eEdD'))...
                       && str2double(x);
	defaultmAlphaA = '35';
	defaultmAlphak = '0.1';
	defaultmAlphad = '-5'; 
    ...
    defaulthAlphaA = '0.225'; 
	defaulthAlphak = '0.1'; 
	defaulthAlphad = '-80'; 
	defaulthBetaA = '7.5'; 
	defaulthBetak = '0.0555556'; 
	defaulthBetad = '3'; 

    addParamValue(p, 'mAlphaA', defaultmAlphaA, numbercheck);
	addParamValue(p, 'mAlphak', defaultmAlphak, numbercheck);
	addParamValue(p, 'mAlphad', defaultmAlphad, numbercheck);
	addParamValue(p, 'mBetaA',  defaultmBetaA,  numbercheck);
	addParamValue(p, 'mBetak',  defaultmBetak,  numbercheck);
	addParamValue(p, 'mBetad',  defaultmBetad,  numbercheck);
    
	addParamValue(p, 'hAlphaA', defaulthAlphaA, numbercheck);
	addParamValue(p, 'hAlphak', defaulthAlphak, numbercheck);
	addParamValue(p, 'hAlphad', defaulthAlphad, numbercheck);
	addParamValue(p, 'hBetaA',  defaulthBetaA,  numbercheck);
	addParamValue(p, 'hBetak',  defaulthBetak,  numbercheck);
	addParamValue(p, 'hBetad',  defaulthBetad,  numbercheck);
    
end

function result = filecheck(x)
    if 1
        if isunix
            OS = OSType.UNIX;
        else
        ...
        xconv = pathConversion(x, OS);
    else
        xconv = x;
    end
    
    if (2 ~= exist(xconv, 'file'))
        error (['File ' xconv ' does not exist.'])
    else
        result = true;
    end
end

function result = dircheck(x)
    [pathstr, ~, ~] = fileparts(x) ;

    if (7 ~= exist(pathstr, 'dir'))
        error (['Directory ' pathstr ' does not exist.'])
    else
        result = true;
    end
