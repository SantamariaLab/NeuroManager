...
% SimNeuronSimpleSpike02A
% A hoc-only Neuron simulator that changes three input parameters by
% constructing parameters.hoc on the host by using a concrete subclass
% method.
classdef SimNeuronSimpleSpike02A < SimNeuron
    properties
        version;
    end
    ...
    methods
        function obj = SimNeuronSimpleSpike02A(id, machine, type, dbH, ...
                                               log, notificationSet)
        ...
        % Abstract is in Sim_Neuron.
            ...
            % need a unique name in the scratch dir but don't want one on
            % the target (or else we'd have to modify runme.hoc).
            hostParameterFilename = [simulation.getID(), 'parameters.hoc'];
            targetParameterFilename = 'parameters.hoc';
            parameterFile = fullfile(obj.machine.getScratchDir(),...
                                     hostParameterFilename);
            pFile = fopen(parameterFile, 'w');
            fprintf(pFile, '%s\n', ['// parameters.hoc for ' simulation.getID()]); 
            fprintf(pFile, '%s\n', '// This file generated automatically; do not edit.'); 
            fprintf(pFile, '%s\n', 'print "parameters: defining variables"'); 
            % Current, leak conductance, and duration values are passed
            % straight through, but could have range/validity checking here
            % if desired. 
            fprintf(pFile, 'icurrent = %s\n', simulation.getParam(1)); 
            fprintf(pFile, 'idelay = %s\n', '100'); 
            fprintf(pFile, 'iduration = %s\n', simulation.getParam(2)); 
            fprintf(pFile, 'tstop = %s\n', '300'); 
            fprintf(pFile, 'dt = %s\n', '0.010'); 
            fprintf(pFile, 'dt2 = %s\n', '0.020'); 
            fprintf(pFile, 'v_init = %s\n', '-65'); 
            fprintf(pFile, 'celsius = %s\n', '37'); 
            fprintf(pFile, 'Input_Leak_gl = %s\n', simulation.getParam(3));
            fprintf(pFile, '%s\n', 'strdef fname');
            dataFile = [simulation.getID() '.dat'];
