...
function [result, errMsg] =...
    ...
    % In Example02B we create our parameter.hoc on the fly with input
    ...
    % Current, leak conductance, and duration values are passed straight
    ...
    if status == 0 % i.e, successful return from the simulation
        ...
        title({['SimpleSpike02B'];...
               ['SimID = ' simID];...
               ['icurrent = ' currentStr ' nA;   duration = ' durationStr]});
        xlabel('Time (msec)');
        ylabel('Voltage (mV)');
        grid on;
        filename = [simID '_current_' currentStr '.fig'];
        saveas(h, fullfile(outputDir, filename), 'fig')
        filename = [simID '_current_' currentStr '.tiff'];
