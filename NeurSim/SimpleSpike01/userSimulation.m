...
function [result, errMsg] =...
    ...
    if status == 0 % i.e, successful return from the simulation
        if (~exist(fullfile(runDir, dataFile), 'file'))
            errMsg = ['Could not find ' dataFile];
            result = 1;
            return;
        else
            copyfile(fullfile(runDir, dataFile), outputDir);
        end
        
        % Plot the data
        load(fullfile(runDir, dataFile));
        h = figure('visible', 'on'); 
        eval(['plot(' data ' (:,1), ' data '(:,2))']);
        title({['SimpleSpike01 Example'];...
               ['simid']});
