...
function [result, errMsg] =...
    ...
    % We are essentially running our PySim function called pyfuncname
    ...
    if status == 0 % i.e, successful return from the simulation
        ...
        % Actually should get the labeling data from simulation command
        % returns, instead of directly from the input, since the simulation
        % may have had to adjust them for compatibility (not implemented
        % yet). Just choose what you want in the title; too much to put
        % everything.
        title({['Miyasho model - somatic stimulation. SimID = ' simID];...
               ['Current = ' current ' nA;   delay = ' delay...
                ' ms; stimduration = ' stimDuration ' ms'];...
               ['Timestep = ' timeStep ' msec; Record interval = '...
                recordInterval ' msec'];
