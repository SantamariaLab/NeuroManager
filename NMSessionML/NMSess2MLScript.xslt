...
<xsl:param name="installDir" select="'.'"/>

<xsl:variable name="customdirpath" select="translate(translate(concat($installDir, '/', //locationinfo/customdir), '//', '/'), '&#x20;&#x9;&#xD;&#xA;', '')" />
<xsl:variable name="scriptoutputpath" select="translate(concat($customdirpath, '/', /nmsession/@id, 'SimSet.m'), '//', '/')" />
<xsl:variable name="simspecfilename" select="concat(/nmsession/@id, 'SimSpec.txt')" />
<xsl:variable name="simspecoutputpath" select="translate(concat($customdirpath, '/', $simspecfilename), '//', '/')" /> 

<xsl:template match="nmsession">
<!-- 	<xsl:message>DEBUG</xsl:message>
	<xsl:message>DEBUG2</xsl:message>
	<xsl:message><xsl:value-of select="concat('+', $installDir, '+')"/></xsl:message>
	<xsl:message><xsl:value-of select="concat('+', $customdirpath, '+')"/></xsl:message>
...
<xsl:template match="locationinfo">
		<xsl:value-of select="normalize-space(concat('[nmAuthData, nmDirectorySet, userData] = ', translate(normalize-space(concat(userinfo ,'();')),' ','')))"/>
		<xsl:text>&#x0A;</xsl:text>
		<xsl:text>nmDirectorySet.customDir = fullfile(nmDirectorySet.nmMainDir, '</xsl:text>
		<xsl:value-of select="translate(customdir,'&#x20;&#x9;&#xD;&#xA;', '')"/>
		<xsl:text>');&#x0A;</xsl:text>
		<xsl:if test="/nmsession/sessionscript/locationinfo/modeldir">
			<xsl:text>nmDirectorySet.modelDir = fullfile(nmDirectorySet.nmMainDir, '</xsl:text>
			<xsl:value-of select="translate(modeldir,'&#x20;&#x9;&#xD;&#xA;', '')"/>
			<xsl:text>');&#x0A;</xsl:text>
		</xsl:if>
		<xsl:text>nmDirectorySet.resultsDir = fullfile(nmDirectorySet.nmMainDir, '</xsl:text>
