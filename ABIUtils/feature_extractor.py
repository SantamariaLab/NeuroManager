...
class EphysFeatureExtractor( object ):
    ...
    def process_instance(self, name, v, curr, t, onset, dur, stim_name):
        ...
        # dvdt = dv / (np.diff(t[start_idx:stop_idx])) # in mV/ms  dbs remove conversion to ms because not consistent with previous
        
        # dbs
        # debugDir = ('C:/Users/David/Dropbox/Documents/'
                    # + 'SantamariaLab/Projects/Fractional/ABI-FLIF/SpikeDev/expData')
        # debugDir = ('C:/Users/David/Dropbox/Documents/'
                    # + 'SantamariaLab/Projects/Fractional/ABI-FLIF/SpikeDev')
        # dvdtFile = debugDir + '/' + 'dVdT.dat'
        # np.savetxt(dvdtFile, dvdt)

        dv_cutoff = 20
        thresh_pct = 0.05
        spikes = []
        temp_spk_idxs = np.where(np.diff(np.greater_equal(dvdt, dv_cutoff).astype(int)) == 1)[0] # find positive-going crossings of 100 mV/ms
        print "temp_spk_idxs", temp_spk_idxs # dbs
        ...
        # dvdt = dv / (np.diff(t)) # in mV/ms  dbs remove conversion to ms  because not consistent with previous
        ...
        for spk_n, spk_idx in enumerate(spk_idxs):
            ...
            # dbs
            print "Printing spk before tests"
            for ky,vl in spk.items():
                print (ky,vl)
            print "Done printing spk" # dbs
            
            print "peak_idx:", peak_idx                 # dbs
            print "v[peak_idx]:", v[peak_idx]           # dbs
            print "t[peak_idx]:", t[peak_idx]           # dbs
            print "threshold_idx:", threshold_idx       # dbs
            print "v[threshold_idx]:", v[threshold_idx] # dbs
            print "t[threshold_idx]:", t[threshold_idx] # dbs
        ...
        # Fill in the characteristics of each spike (dbs comment)
