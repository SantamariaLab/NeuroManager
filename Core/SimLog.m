...
% simlog
% A class which provides logging facilities for NeuroManager.
classdef SimLog < handle
    % For reliability in case of failure, we simply open and close the file
    % every entry. Input strings can be as long as desired (check MATLAB
    % limits though). 
    properties
        logPathname = '';
        logFileHandle;
        % For echoing log output to the terminal
        echoFlag = true;
    end
    ...
    methods
        % ----------------
        % Constructor
        function obj = SimLog(fullPathname, echoFlag)
            obj.logFileHandle = fopen(fullPathname, 'w');
            if (obj.logFileHandle == -1)
                ME = MException('ClassdefSimLog:LogFileFailure', ...
                                ['NeuroManager: Could not create ' fullPathname]);
                throw(ME);
            end
            obj.logPathname = fullPathname;
            obj.echoFlag = echoFlag;
            msg = 'SimLog created';
            dStr = datestr(now);
        ...
        % Writes the supplied message to the log
            obj.logFileHandle = fopen(obj.logPathname, 'a');
            if (obj.logFileHandle == -1)
                ME = MException('ClassdefSimLog:LogFileOpenFailure', ...
                                ['NeuroManager: Could not open ' obj.logPathname]);
                throw(ME);
            end
            dStr = datestr(now);
            fprintf(obj.logFileHandle, '%s - %s\n', dStr, msg);
