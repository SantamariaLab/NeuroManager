...
% processSimSpec.m
% Acts on the input simspec to set various globals and add simulations.
% Part of the SimSet class definition. 
function [result, errmsg] = processSimSpec(obj, simspec)
    errmsg = '';
    tokenArray = simspec.getData();
    % Process the SimSpec
    for i=1:simspec.getNumLines();
        % if the line is empty then skip it.
        % If first char is a '%' then it's a comment line and skip it
        command = tokenArray(i,1).command;
        if((numel(command) == 0) || (command(1) == '%'))
            continue;
        end
        cmdParam = tokenArray(i,1).cmdParam; 
        ...
        switch command
            case 'SIMSETDEF'
                obj.setID(cmdParam);
                if isempty(obj.id)
                    ME = MException('ClassdefSimSet:ProcessSimSpec', ...
                        'SIMSETID must be specified.');
                    throw(ME);
                end
                typestr = tokenArray(i,1).params{1,1};
                try
                    type = SimType.(typestr); % Dynamic field name
                catch
                      ME = MException('ProcessSimSpec:InvalidSimType',...
                                      ['ProcessSimSpec: Invalid SimType: <'...
                                       typestr '> in line ' num2str(i)...
                                       ' of simspec file.']);
                      throw(ME);
                end
                obj.setType(type);
            % ---
            case {'SIMDEF', 'SIMDEFN'}
                % Check here to ensure that ID is already set (otherwise
                % the directory construction will not work properly)
                % (not implemented yet)
                numParams = min(size(tokenArray(i,1).params, 2), obj.MAX_PARAMS);
                if strcmp(command, 'SIMDEF')
                    obj.addSimDef(cmdParam, obj.getType(), false, ...
                                  {tokenArray(i,1).params{1,1:numParams}});
                else
                    obj.addSimDef(cmdParam, obj.getType(), true, ...
                                  {tokenArray(i,1).params{1,1:numParams}});
                end
            % ---
            otherwise  
                result = 1; % 1 is failure at the moment
                errmsg = command;
                return;
        end
    end
