...
% importSimSpec.m
% Reads in a SimSpec from a named file and returns an array of token
% structs to be processed. Each line corresponds to one row in simspec.
% Currently limited to MAX_PARAMS parameters but unlimited lines.
% Part of the SimSet classdef.
function simspec = importSimSpec(obj, filename) 
    % Check for valid filename
    A = exist(filename, 'file');
    if (A ~= 2)
        ME = MException('ClassdefNotificationSet:ImportSimSpecFileNotExist', ...
                        ['SimSet: SimSpecFile ' strrep(filename, '\', '\\') ...
                         ' not found.']);
        throw(ME);
    end
    ...
    for i = 1:length(inputLines)
        [command, remain]  = strtok(inputLines{1,i}); %#ok<*AGROW>
        [cmdParam, remain] = strtok(remain); %#ok<*STTOK>
        params = {};
        for j=1:(obj.MAX_PARAMS)
            [token, remain]  = strtok(remain);
            params(1,j) = {token};
        end
        % Assume possibility that simids are not controlled properly and
        % test for uniqueness
        overrideUniquenessCheck = false;
        simspec.addTokenSet(command, cmdParam, params, overrideUniquenessCheck);
    end
end
