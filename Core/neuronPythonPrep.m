...
% 	fprintf(f, '%s\n', ['#!' md.getSetting('pythonPath')]);
    ...
    % simulation-specific data (necessary since the nrniv command does not
...
%     argumentstr = ['''', strjoin(arguments, ''', '''), ''''];
    if (~isempty(arguments))
        argumentStr = ['''' arguments{1}];
        for i= 2:length(arguments)
            argumentStr = [argumentStr ''', ''' arguments{i}]; %#ok<AGROW>
        end
        argumentStr = [argumentStr ''''];
        fprintf(f, '%s', argumentStr);
    end
    fprintf(f, ')\n');
    fprintf(f, 'sys.exit(result)\n');
    fclose(f);
    copyfile(wrapPath, outputDir); % for documentation/debug
end
