...
function str = getMATLABCompileVersion(obj)
    if(obj.xCompilationMachine == 0)
        command = [path2UNIX(fullfile(obj.matlabCompilerDir,...
                                      obj.matlabExecutable))...
                   ' -nojvm -nosplash -nodisplay '...
                   '-r "v=version;disp([''Version:'' v]);exit"'...
                   '|grep Version'];
         result = obj.issueMachineCommand(command, CommandType.JOBSUBMISSION);
         str = strrep(result{1}, 'Version:', '');
         str = [str ' on machine ' obj.getID()];
    else
        command = [path2UNIX(fullfile(obj.xCompilationMachine.matlabCompilerDir,...
                                      obj.xCompilationMachine.matlabExecutable))...
                   ' -nojvm -nosplash -nodisplay '...
                   '-r "v=version;disp([''Version:'' v]);exit"'...
                   '|grep Version'];
         result = obj.xCompilationMachine.issueMachineCommand(command, CommandType.JOBSUBMISSION);
         str = strrep(result{1}, 'Version:', '');
         str = [str ' on machine ' obj.xCompilationMachine.getID()];
    end
end
