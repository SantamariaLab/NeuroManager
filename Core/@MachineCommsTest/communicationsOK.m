...
% -----------------
function tfResult = communicationsOK(obj)
    obj.log.write(['Beginning basic communications test.']);
    % Now do comms checks
    % The Java library will give an authentication error if it
    % can't connect with ssh2.
    command = 'echo 42';
    result{1} = ''; %#ok<NASGU>
    result = obj.issueMachineCommand(command, CommandType.FILESYSTEM);
    if ~strcmp(result{1}, '42')
        obj.log.write(['Communications check failed with FILESYSTEM IP of '...
                       obj.id ' machine.']);
        tfResult = false;
        return;
    end
    ...
    if ~strcmp(result{1}, '43')
     obj.log.write(['Communications check failed with JOBSUBMISSION IP of '...
         obj.ID ' machine.']);
        tfResult = false;
        return;
    end
    tfResult = true;
    obj.log.write(['Basic communications test passed.']);
end
