...
% fileTransferOK
% Part of the Machine_CommsTest class definition.
% -----------------
% This is serial so no need to make filenames unique; we make the
% content unique to add in post-mortem debug.
% This only tests file transfer to/from the FILESYSTEM machine.
% Normal file operations are not verified like this.
function tfResult = fileTransferOK(obj)
    testName = 'commstest.txt';
    testFile = fullfile(obj.machineScratchDir, testName);
    targetFile = fullfile(obj.targetBaseDir, testName);

    testName2 = 'commstest2.txt';
    testFile2 = fullfile(obj.machineScratchDir, testName2);
    targetFile2 = fullfile(obj.targetBaseDir, testName2);

    %%  Synchronous file transfer test
    % 1) Create a file, upload it, move it, download it.
    obj.log.write(['Beginning synchronous file transfer test.']);

    % Ensure clear initial conditions
    if exist(testFile, 'file'); delete(testFile); end
    if exist(testFile2, 'file'); delete(testFile2); end
    command = ['mv ' path2UNIX(obj.targetBaseDir)...
               ';rm '  path2UNIX(targetFile)...
               ';rm '  path2UNIX(targetFile2)];
    obj.issueMachineCommand(command, CommandType.FILESYSTEM);
    result = obj.checkForCheckfile(targetFile);
    if result
        tfResult = false;
        failpointIndicator = 'S1';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    end
    result = obj.checkForCheckfile(targetFile2);
    if result
        tfResult = false;
        failpointIndicator = 'S2';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    ...
    % Create the upload file, upload it, and verify
    f = fopen(testFile, 'w');
    fprintf(f, '%s\n', obj.id);  % id for debug only
    fclose(f);
    if ~exist(testFile, 'file')
        tfResult = false;
        failpointIndicator = 'S3';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    end
    obj.fileToMachine(testFile, targetFile);
    result = obj.checkForCheckfile(targetFile);
    if ~result
        tfResult = false;
        failpointIndicator = 'S4';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    ...
    % Move to name two and verify
    command = ['mv ' path2UNIX(targetFile)...
               ' '  path2UNIX(targetFile2)];
    obj.issueMachineCommand(command, CommandType.FILESYSTEM);
    result = obj.checkForCheckfile(targetFile);
    if result
        tfResult = false;
        failpointIndicator = 'S5';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    ...
    % Download file 2 and verify
    obj.fileFromMachine(obj.machineScratchDir, targetFile2);
    if ~exist(testFile2, 'file')
        tfResult = false;
        failpointIndicator = 'S7';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Synchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    ...
    if exist(testFile, 'file'); delete(testFile); end
    if exist(testFile2, 'file'); delete(testFile2); end
    obj.log.write(['Synchronous file transfer test passed.']);

    %%  Asynchronous file transfer test
    % 2) Upload a file, rename it, and download it in the
    % background
    obj.log.write(['Beginning asynchronous file transfer test.']); %#ok<*NBRAK>
    % Clear the target
    command = ['mv ' path2UNIX(obj.targetBaseDir)...
               ';rm '  path2UNIX(targetFile)...
               ';rm '  path2UNIX(targetFile2)];
    ...
    if ~exist(testFile, 'file')
        tfResult = false;
        failpointIndicator = 'A1';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Asynchronous file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    end
    obj.fileToMachine(testFile, targetFile);
    ...
    signalFilename = 'TESTASYNCDOWNLOADCOMPLETE';
    errorFilename = 'TESTASYNCDOWNLOADERROR';
    signalFile = fullfile(obj.machineScratchDir, signalFilename);
    errorFile  = fullfile(obj.machineScratchDir, errorFilename);
    if exist(signalFile, 'file'); delete(signalFile); end
    if exist(errorFile, 'file'); delete(errorFile); end
    % LIMITED BECAUSE OF HARDWIRED TIMEOUT OF ~THREE MINUTES
    fileFromMachineNoWait(obj, obj.machineScratchDir, targetFile2,...
                          signalFilename, errorFilename);
    % Network delays can put these out of sync so we have to wait
    % for both.
    i = 0;
    tfResult = true;
    while ((~exist(signalFile, 'file')) || ...
           (~exist(testFile2, 'file')))
        if exist(errorFile, 'file')
            tfResult = false;
            failpointIndicator = 'A5';
            obj.touchMsg(failpointIndicator);
            obj.log.write(['Asynchronous file transfer test failed at '...
                           failpointIndicator '.']);
            return;
    ...
    if exist(signalFile, 'file'); delete(signalFile); end
    if exist(errorFile, 'file'); delete(errorFile); end
    batchFile = fullfile(obj.machineScratchDir, 'Download.bat');
    if exist(batchFile, 'file'); delete(batchFile); end
    obj.log.write(['Asynchronous file transfer test passed.']);

    %%  Multiple file upload test
    % 3) Upload a file list and check the target for their arrival
    obj.log.write(['Beginning multiple file upload test.']);
    ...
    if ~exist(testFile, 'file')
        tfResult = false;
        failpointIndicator = 'M1';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Multiple file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    end
    copyfile(testFile, testFile2, 'f')

    obj.fileListToMachine({testName, testName2},...
              obj.machineScratchDir, obj.targetBaseDir);
    result = obj.checkForCheckfile(targetFile);
    if ~result
        tfResult = false;
        failpointIndicator = 'M2';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Multiple file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    ...
    if ~result
        tfResult = false;
        failpointIndicator = 'M3';
        obj.touchMsg(failpointIndicator);
        obj.log.write(['Multiple file transfer test failed at '...
                       failpointIndicator '.']);
        return;
    end
    obj.log.write(['Multiple file upload test passed.']);

    % Clean up to provide clean directories for next set of tests
    command = ['mv ' path2UNIX(obj.targetBaseDir)...
               ';rm '  path2UNIX(targetFile) ...
               ';rm '  path2UNIX(targetFile2)];
    obj.issueMachineCommand(command, CommandType.FILESYSTEM);
    if exist(testFile, 'file'); delete(testFile); end %#ok<*SEPEX>
    if exist(testFile2, 'file'); delete(testFile2); end
    if exist(signalFile, 'file'); delete(signalFile); end
    if exist(errorFile, 'file'); delete(errorFile); end
    batchFile = fullfile(obj.machineScratchDir, 'Download.bat');
