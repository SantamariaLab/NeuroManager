...
% library located off the inputdir and the python wrapper at pythonwrappath
% for the machine type seen in machineid. Assumes bash.
function shellName = createPythonNrnivsh(simCore, runDir, inputDir,...
                                         outputDir, pythonWrapPath)
    f = fopen(fullfile(runDir, 'nrnivsh.sh'), 'w');
    fprintf(f, '#!/bin/bash\n');
    
%     fprintf(f, '%s\n',...
%         ['export NEURONHOME=' fullfile(md.getSetting('neuronDir'),...
%                                        md.getSetting('neuronHomeExt'))]);
    fprintf(f, '%s\n',...
        ['export NEURONHOME=' fullfile(simCore.config.neuronDir,...
                                       simCore.config.homeExt)]);
%     fprintf(f, '%s\n',...
%         ['export PATH=' fullfile(md.getSetting('neuronDir'),...
%                                  md.getSetting('neuronBinExt')) ':$PATH']);
    fprintf(f, '%s\n',...
        ['export PATH=' fullfile(simCore.config.neuronDir,...
                                 simCore.config.binExt) ':$PATH']);

% 	fprintf(f, '%s\n',... 
%         ['export LD_LIBRARY_PATH=' fullfile(md.getSetting('neuronDir'),...
%                                    md.getSetting('neuronLibExt'))...
%                                    ':$LD_LIBRARY_PATH']);
	fprintf(f, '%s\n',... 
        ['export LD_LIBRARY_PATH=' fullfile(simCore.config.neuronDir,...
                                   simCore.config.libExt)...
                                   ':$LD_LIBRARY_PATH']);

%     fprintf(f, '%s\n', cell2mat(md.getSetting('envAddlLibLines')));
    fprintf(f, '%s\n', cell2mat(simCore.config.envAddlLibLines));
    
	fprintf(f, '%s\n', ['export PYTHONPATH=' inputDir ':$PYTHONPATH']);

%     addlEnvLines = md.getSetting('pythonEnvAddlLines');
    addlEnvLines = simCore.config.pythonEnvAddlLines;
    if ~isempty(addlEnvLines)
        for i=1:length(addlEnvLines)
            fprintf(f, '%s\n', strrep(addlEnvLines{i}, '''', ''''''));
        end
    end
    
    % Insert the neuron call via the wrapper.  Specifying the mechanisms
    % here is necessary rather than using load_mechanisms because the
    ...
    fprintf(f, '%s\n', ['cd ' runDir '; ']);
%     fprintf(f, '%s\n', ['nrniv' ...
%                         ' -nogui -nobanner -python -dll "'...
%                         fullfile(inputDir,...
%                             md.getSetting('neuronCompModLibExt'),...
%                             'libnrnmech.so') '" '...
%                             pythonWrapPath ...
%                         ' 1> ' ...
%                         fullfile(outputDir,...
...
%                             ]);
	...
	fprintf(f, '%s\n', ['result=$?']);
% 	fprintf(f, '%s\n', ['if [ $result -eq 0 ]']);
% 	fprintf(f, '%s\n', ['then touch NRNIVSUCCESS']);
% 	fprintf(f, '%s\n', ['else touch NRNIVFAILURE']);
% 	fprintf(f, '%s\n', ['fi']);
	fprintf(f, '%s\n', ['echo $?']);
% 	fprintf(f, '%s\n', ['']);
    fclose(f);
    shellName = 'nrnivsh.sh'; 
end
