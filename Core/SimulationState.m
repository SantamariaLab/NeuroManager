...
% SimulationState
% A class which defines the states in which a Simulation can be. 
classdef SimulationState
    properties (Constant)
        % For use in empty arrays.
        UNASSIGNED    = 0;
        
        % The simulation is available to run but hasn't been run yet
        UNRUN         = 1;

        % The simulation has been checked out and is in the works
        INPROGRESS    = 2;
        
        % The simulation job has been given to the OS/Job Manager
        SUBMITTED     = 3;
        
        % The simulation itself indicates that it is running
        RUNNING       = 4;
        
        % The target-side simulation executable has finished running
        RUNCOMPLETE   = 5;
        
        % Any post-completion activities are done
        READYFORDOWNLOAD = 6;
        
        % Download has been initiated but process is not yet complete
        DOWNLOADINPROGRESS = 7;

        % All sim files have been downloaded and remote cleaned up
        FULLYPROCESSED = 8;
        
        % NOT IN USE YET ------------------
        % The sim has not been run but is not available for checkout for
        % some reason
