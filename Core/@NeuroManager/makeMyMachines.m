...
% makeMyMachines
% Part of the NeuroManager class; this class sets up the machines for
...
function mset = makeMyMachines(obj, scratchDir, inSimType, auth)
    mset = {};
    validMachineCount = 0;
    for i = 1:obj.machineSetConfig.getNumMachines()
        [type, numSims, name, config,...
         ~, ~, ~, numNodes,...
         baseDir, wallClockTime, ipAddr, ~] = obj.machineSetConfig.getMachine(i);
        % If no simulators requested, we do not build the machine
        if numSims
            validMachineCount = validMachineCount+1;
            % Check the machine to see if it supports any of the requested
            % SimCores
            simCoreCompatible = false;
            for j=1:length(inSimType.simCoreList)
                if ~any(strcmp(inSimType.simCoreList(j), config.simCores))
                    simCoreCompatible = true;
                    break;
                end
            end
            if simCoreCompatible
                mset{validMachineCount} =...
                    type.constrFunc(wallClockTime,...
                                obj.hostMachineData.id,...
                                obj.hostMachineData.osType,...
                                name,...
                                ipAddr,...
                                baseDir, scratchDir,...
                                obj.simCoreDir, obj.customSimDir,...
                                obj.modelFileDir, inSimType, numSims,...
                                0,'',...
                                auth, obj.log, obj.simNotificationSet,...
                                config,...
                                '', '', '',...
                                numNodes); %#ok<AGROW>
            else
                error(['Machine' name 'does not have SimCore ' ...
                       'compatibility with Simulator Type']);
            end
        end
    end
end
