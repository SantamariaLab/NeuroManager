...
% neuroManagerInputParser
% Parses and validates the NeuroManager constructor parameters
% Part of the NeuroManager class definition.
function p = neuroManagerInputParser(obj) %#ok<INUSD>
    p = inputParser;
    p.StructExpand = true;
    p.CaseSensitive = true;
    p.KeepUnmatched = false;
    validNotificationTypes = {'BOTH', 'TEXT', 'EMAIL', 'OFF', 'NONE'};
    notificationsCheck = @(x) any(validatestring(x, validNotificationTypes));
    
    defaultNMMainDir = '.';
    defaultCoreDir = './Core';
    defaultSSHLibDir = './SSHLib';
    defaultCustomDir = '.';
    defaultModelDir = '.';
    ...
    addParamValue(p, 'authFile', defaultAuthFile, @fileCheckemptyok);
    if ispc
        addParamValue(p, 'puttyAuthFile', defaultPuttyAuthFile,...
                                          @fileCheck);
    else 
        % No verification if not a PC host since they don't use Putty
        addParamValue(p, 'puttyAuthFile', defaultPuttyAuthFile);
    end
    ...
    defaultSingleMachine = false;
    addParamValue(p, 'isSingleMachine', defaultSingleMachine,   @islogical);

    defaultUseDualKey = false;
    addParamValue(p, 'useDualKey', defaultUseDualKey,   @islogical);
    
    defaultShowWebPage = true;
    addParamValue(p, 'showWebPage', defaultShowWebPage,   @islogical);
end
...
% Permits skipping key checks if not going to be used
function result = fileCheckemptyok(x)
    if isempty(x)
        result = 1;
    else
        result = fileCheck(x);
...
function result = dirCheck(x)
    ...
    xconv = pathConversion(x, os);
    if (7 ~= exist(xconv, 'dir'))
        error (['Directory ' xconv ' does not exist.'])
    else
        result = true;
    end
end

function result = dirCheckEmptyOK(x)
    if isempty(x)
        result = true; return;
    else
        if isunix
