...
%                         fullfile(md.getSetting('neuronDir'),...
...
%     addlEnvLines = md.getSetting('envAddlLines');
	...
	% Insert the neuron call.  Specifying the mechanisms
...
%                           'libnrnmech.so');
    ...
    % it isn't necessary (i.e., no custom mod files to deal with) and run
    % neuron without it. Problems with model file compilation would have
    % been caught earlier.
    if (exist(mechanisms, 'file')==2)
        fprintf(f, '%s\n', ['cd ' runDir '; '...
                            'nrniv  -nogui -nobanner '...
                            '-dll "' mechanisms '" '...
                            hocFile ...
                            ' 1> ' ...
                            fullfile(outputDir,...
