...
% SimSpec
% Class which holds and supports a simspec struct for the purpose of
% creating a SimSet. NeuroManager interacts with a SimSet for running, not a
% SimSpec. Note: does not deal with MaxNumParams.
classdef SimSpec < handle
    properties
        data; % array of simstructs
        numLines;
    end
    
    methods
        function obj = SimSpec()
            obj.data = struct('command', '',...
                              'cmdParam', '', 'params', {{}});
            obj.numLines = 0;
        end
        
        function print(obj, printPath)
            [f, msg] = fopen(printPath, 'w');
            if ~isempty(msg)
                err = MException('NeuroManager:SimSpec:FileOpenProblem',...
                    ['Could not open file '...
                     strrep(printPath, '\','\\') ' for printing']);
                throw(err);
            end
            fprintf(f, 'SimSpec Print\n');
            fprintf(f, 'Note: any comments do not appear in this file.\n');
            fprintf(f, '-------------\n');
            fprintf(f, 'numLines: %d\n', obj.numLines);
            for i=1:obj.numLines
                fprintf(f, 'Line %d: %s %s | ', i,...
                        obj.data(i).command, obj.data(i).cmdParam);
                for j = 1:length(obj.data(i).params)
                    fprintf(f, '%s ', obj.data(i).params{j});
                end
                fprintf(f, '\n');
            end
            fclose(f);
        ...
        function addTokenSet(obj, command, cmdParam, params, uniqueOverride)
        % params is a cell array of strings
            if islogical(uniqueOverride)
                override = uniqueOverride;
            else
                override = false;
            end
            temp = struct('command', '', 'cmdParam', '', 'params', {{}});
            temp.command = command;
            temp.cmdParam = cmdParam;
            temp.params = params;
            if ~override
                % If not a comment or an empty line, check for duplicates; if
                % none, add the line
                trimCommand = strtrim(command);
                if ((~strncmp(trimCommand, '%', 1)) && (~isempty(trimCommand)))
                    simids = {obj.data(:).cmdParam};
                    if isempty(setdiff({temp.cmdParam}, simids))
                        error(['SimSpec Error:All ids of simset and simulations must be unique: ' temp.cmdParam]);
                    end
                    obj.addLines(temp);
                end
            else
        ...
        function num = getNumLines(obj)
            num = obj.numLines;
        end
    end
    
    methods(Access=private)
        function addLines(obj, simstructArray)
        % Add multiple lines to the Data
            numLines = size(simstructArray, 1);
            % No need for error; if it's empty, just don't add anything
            if numLines > 0 
                obj.data(obj.numLines+1:obj.numLines+numLines, 1) = simstructArray; %#ok<*PROP>
                obj.numLines = obj.numLines + numLines;
            end
        end
    end
end
