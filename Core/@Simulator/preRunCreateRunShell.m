...
% preRunCreateRunShell.m
% Create the shell file for the simulation run, upload it to the
% Simulator's basedir, and make it executable. 
% Part of the Simulator class.
% ------------
function preRunCreateRunShell(obj)
    ...
    runshname = 'SimRunShell.sh';
    runshpath = fullfile(simulation.getHostBaseDir(), runshname);
    f = fopen(runshpath, 'w');
    fprintf(f, '%s\n', ['#!/bin/bash']);
    fprintf(f, '%s\n', ['# This script is automatically generated '...
                        'and will be overwritten.']);
    % If the model compilation failed, then we short-circuit to
    % simulation failure with appropriate message. If model
    % compilation succeeded, or if there is no model compilation,
    % then the MODCOMPILEFAILURE will not appear. So we can use
    % this same script for both types (for all simulators). IDEALLY
    % - would have a more positive check built in (for future
    % development). 
    fprintf(f, '%s\n', ['if [ -e '...
                        path2UNIX(fullfile(indir, 'MODCOMPILEFAILURE'))...
                        ' ] ']);
    fprintf(f, '%s\n', ['then']);
    fprintf(f, '%s\n', ['   printf ''ID ' simulation.getID() '\n'...
                        'Result FAILED\n'...
                        'Simtime 0.00\n'...
                        'Errmsg Model file compilation failure. Check '...
                        'nrnivmodlcompile.txt and '...
                        'stderrnrnivmodlcompile.txt.\n'...
                        'MATLAB Version MATLAB not reached.\n'' > '...
                        path2UNIX(fullfile(outdir, 'SimulationResults.txt'))]);
    fprintf(f, '%s\n', ['   cd ' path2UNIX(outdir)...
                        '; zip -q ' simulation.getID() ' *; cd '...
                        path2UNIX(obj.targetBaseDir)]);
    fprintf(f, '%s\n', ['   touch ' path2UNIX(fullfile(outdir, 'FAILED'))]);
    fprintf(f, '%s\n', ['   echo 1']);
    fprintf(f, '%s\n', ['   exit 1']);
    % If the model compilation didn't fail, run the simulation. If
    % simulation run fails outside of the code execution itself
    % (such as a MATLAB syntax error), then do the same thing an
    % internal programmatic failure would have.
    fprintf(f, '%s\n', ['else']);
    mcrDir = path2UNIX(obj.machine.getMcrDir());
    commandstr =...
           ['./run_runSimulation.sh '...
            mcrDir ' '''...
            obj.machine.getID()...
            ''' ''' ...
            ...
            ''' ''' ...
            path2UNIX(outdir)...
            ''' ' obj.fetchSimulationParamStr() ...
            ' 1> '...
            path2UNIX(...
            fullfile(outdir, 'stdout.txt'))...
            ' 2> '...
            path2UNIX(...
            fullfile(outdir, 'error.txt'))];
    fprintf(f, '%s\n', ['   ' commandstr]);
    fprintf(f, '%s\n', ['   result=$?']);
    fprintf(f, '%s\n', ['   if [ $result -eq 0 ]']);
    fprintf(f, '%s\n', ['       then touch '...
        path2UNIX(fullfile(outdir, 'COMPLETE'))]);
    fprintf(f, '%s\n', ['   else']);
    fprintf(f, '%s\n', ['      printf ''ID ' simulation.getID() '\n'...
                        'Result FAILED\n'...
                        'Simtime 0.00\n'...
                        'Errmsg Runtime simulation failure. Check '...
                        'error.txt and other *.txt files in the '...
                        'downloads for debugging information.\n'...
                        'MATLAB Version (Runtime version not available for this failure mode).\n'' > '...
                        path2UNIX(fullfile(outdir, 'SimulationResults.txt'))]);
    fprintf(f, '%s\n', ['      cd ' path2UNIX(outdir)...
                        '; zip -q ' simulation.getID() ' *; cd '...
                        path2UNIX(obj.targetBaseDir)]);
    ...
    fprintf(f, '%s\n', ['exit $?']);
    fclose(f);
    targetPath = fullfile(obj.getTargetBaseDir(), runshname);
    obj.machine.fileToMachine(runshpath, targetPath);
    command = ['cd ' path2UNIX(obj.getTargetBaseDir())...
               '; chmod +x ' runshname];
