...
% passwords will override dual key and use the password instead for that
% machine.
% If usedualkey == false, then empty passwords are not acceptable and the
% program will abort with a descriptive error message.
classdef SimAuthenticate < handle
    ...
    methods
        function obj = SimAuthenticate(singleMachine, useDualKey,...
                                       keyFile, puttyKeyFile, hostOS,...
                                       agentDir, log)
            ...
            % If singleMachine, no authentication setup
            if obj.singleMachine
                obj.log.write(['Single Machine Authentication (no authentication) requested.'...
                    '  All passwords and key settings will be ignored.']);
                return;
            end
            
            % Set up Dual Key if it has been requested
            if obj.useDualKey
                obj.log.write(['Dual Key Authentication Services requested.'...
                ...
                if hostOS == OSType.WINDOWS
                    ...
                    system(['source ' fullfile(agentDir, 'agent-info')...
                            ';ssh-add ' keyFile]);
                    obj.agentInfo = fullfile(agentDir, 'agent-info');
                end
            else
                obj.log.write(['Password--based Authentication requested.'...
        ...
        function tf = isUseDualKey(obj)
            tf = obj.useDualKey;
        end
        
        function str = getKeyFile(obj)
            str = obj.keyFile;
        end
        
        function str = getPuttyKeyFile(obj)
            str = obj.puttyKeyFile;
        end
        
        function str = getKeyPassphrase(obj)
            str = obj.keyPassphrase;
        end
        ...
        function delete(obj)
            if ~obj.singleMachine
                if obj.hostOS == OSType.UNIX
                    if obj.useDualKey
                        system(['source ' obj.agentInfo ...
                                '; kill "$SSH_AGENT_PID"']);
                    end
                end
            end
