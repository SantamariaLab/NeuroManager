...
% SLURMMachine
% Adds to RunJobMachine facilities that handle job submission through the
% SLURM batch facility (sbatch command).
% Need to validity-check wall clock time and check it against tacc limits
% in the appropriate classes.
classdef SLURMMachine < RunJobMachine
    properties (Abstract)
    ...
    methods
        % --------------
        % Constructor
        function obj = SLURMMachine(machineData,...
                                    xCmpMach, xCmpDir,...
        ...
        % (immediate) UNIX return of the command in question
        ...
        function jobFilename = preRunCreateJobFile(obj, scratchDir, jobRoot,...
            ...
            % We use the machine scratch directory to
            % construct it
