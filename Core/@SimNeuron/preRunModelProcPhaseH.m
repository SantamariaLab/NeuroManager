...
function preRunModelProcPhaseH(obj) 
    ...
    if obj.noModFiles
        ...
        fprintf(f, '%s\n', ['export PATH=' ...
            path2UNIX(obj.machine.getNeuronBinPath())...
            ':$PATH']);
        fprintf(f, '%s\n', ['export LD_LIBRARY_PATH='...
            path2UNIX(obj.machine.getNeuronLibPath())...
            ':$LD_LIBRARY_PATH']);
        fprintf(f, '%s\n', ['export NEURONHOME='...
            path2UNIX(obj.machine.getNeuronHomePath())]);

        fprintf(f, '%s\n', ['nrnivmodl 1> '...
             path2UNIX(fullfile(outDir,...
                                                'nrnivmodlcompile.txt'))...
             ' 2> '...
             path2UNIX(fullfile(outDir,...
                                                'stderrnrnivmodlcompile.txt'))]);
        % (Successful completion is indicated by the presence of the
