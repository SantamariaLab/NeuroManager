...
% SimNeuron
% Base class for all Neuron simulations. Note: the
...
classdef SimNeuron < ModelFileSim
    ...
    methods (Abstract)
        % Modifying and/or creating on the host, then uploading individual
        % mod/hoc files that depend on simulation input parameters are done
        % by this function.  This is where inserts such as ion channels are
        % done; since they are decided by a specific model the function is
        % abstract here and made concrete by subclasses.
        preRunModelProcPhaseHModFileModification(obj)
    ...
    methods
        function obj = SimNeuron(id, machine, type, dbH, ...
                                                    log, notificationSet)
            obj = obj@ModelFileSim(id, machine, type, dbH, ...
                                                    log, notificationSet);
        end
        ...
        % Override the ModelFileSim version
            ...
            versionStr = result{1};
            obj.log.write(['Neuron Version for '...
            ...
            result = obj.machine.issueMachineCommand(command,...
                                            CommandType.JOBSUBMISSION);
            versionStr = result{1};
            obj.log.write(['Python Version for '...
                           obj.getID() ': ' versionStr]);
        end
        ...
        function removeRemoteAspect(obj)
            command = ['cd '...
                        ...
                        '; rm ' path2UNIX(fullfile(obj.targetBaseDir, '*.mod'))...
                        '; rm ' path2UNIX(fullfile(obj.targetBaseDir, '*.hoc'))...
                        '; rm ' path2UNIX(fullfile(obj.targetBaseDir, '*.py'))...
                        '; rm ' path2UNIX(fullfile(obj.targetBaseDir, '*.sh'))...
