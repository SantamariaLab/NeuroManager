...
% RMD.m
% The basis for the Create...Data.m files that instantiate the data
% associated with a Real Machine. (RMD - Real Machine Data)
classdef RMD < handle
    properties
        keys = {'resourceCategory',...
                'resourceName',...
                'instanceName',...
                'hostKeyFingerprint',...
                'OS_IdentityEndpoint',...
                ...
                'pythonEnvAddlLines',...
                'envAddlLines'};
        settings;  % map container of setting/value pairs
    end
    methods
        function obj = RMD()
            n = length(obj.keys);
            ...
            obj.settings = containers.Map(obj.keys, default, 'UniformValues',false);
        end
        
        function addSetting(obj, name, value)
            if (isKey(obj.settings, name))
                obj.settings(name) = value;
            end
        end
        
        function tf = isSetting(obj, name)
            tf = isKey(obj.settings, name);
        end
        
        function value = getSetting(obj, name)
            value = obj.settings(name);
        end
        
        function settingsMap = getSettingsMap(obj)
            settingsMap = obj.settings;
        end
    end
end
