...
% PathConversion.m
% Converts a path to the format required by newOS.
% A (too?) simplistic function that swaps the slashes around.
% Handles only UNIX and not-UNIX at the moment.
function newpath = pathConversion(oldPath, newOS)
   if (newOS == OSType.UNIX)
