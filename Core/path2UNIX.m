...
% Path2UNIX.m
% Converts a path to UNIX format for use on a target machine.
% NeuroManager policy tries to be that path conversions occur upon input and output
% to/from the NeuroManager routines; internal path format is handled
% automatically by the MATLAB installation, mostly through the fullfile
% function, and so for all host routines is stored with the host format.
% Generally the use of a fullfile is an alert to consider using a
% path conversion, since most fullfiles are done on the host.
function newpath = path2UNIX(oldpath)
