...
% sendTextMsg
% Sends a text message to the user with the specified subject and message
% and optional attachment. 
% Part of the NotificationSet class. Not all carriers tested.
% Original code by Ke Feng, Sept. 2007 in the MATLAB apps online
function result = sendTextMsg(obj, subject, message, attachment)
    % Format the phone number to 10 digit without dashes
    number = strrep(obj.notifications.address, '-', '');
    ...
    % also http://www.netrafic.com/blog/email-to-cell/
    extcarrier = strrep(strrep(lower(obj.notifications.carrier),'-',''),'&','');
    if ~isempty(attachment)
        extcarrier = [extcarrier 'mms'];
    end
    % NOTE: MOST OF THESE HAVE NOT BEEN TESTED
    switch extcarrier
        case 'alltel';     emailto = strcat(number,'@message.alltel.com');
        case 'alltelmms';  emailto = strcat(number,'@mms.alltelwireless.com');
            
        case 'att';        emailto = strcat(number,'@txt.att.net');
        case 'attmms';     emailto = strcat(number,'@mms.att.net');
            
        case 'boost';      emailto = strcat(number,'@myboostmobile.com');
        case 'boostmms';   emailto = strcat(number,'@myboostmobile.com');
            
        case 'cingular';   emailto = strcat(number,'@mobile.mycingular.com');
        case 'cingularmms'; emailto = strcat(number,'@mms.mycingular.com');
            
        case 'nextel';     emailto = strcat(number,'@messaging.nextel.com');
        case 'nextelmms';  emailto = strcat(number,'@messaging.nextel.com');

        case 'sprint';     emailto = strcat(number,'@messaging.sprintpcs.com');
        case 'sprintmms';  emailto = strcat(number,'@pm.sprint.com');

        case 'tmobile';    emailto = strcat(number,'@tmomail.net');
        case 'tmobilemms'; emailto = strcat(number,'@tmomail.net');

        case 'verizon';    emailto = strcat(number,'@vtext.com');
        case 'verizonmms'; emailto = strcat(number,'@vzwpix.com');
            
        case 'virgin';     emailto = strcat(number,'@vmobl.com');
        case 'virginmms';  emailto = strcat(number,'@vmobl.com');
    end
    ...
    setpref('Internet','E_mail',        obj.notifications.authEmail);
    setpref('Internet','SMTP_Server',   obj.notifications.smtpServer);
    setpref('Internet','SMTP_Username', obj.notifications.authEmail);
    setpref('Internet','SMTP_Password', obj.notifications.smtpPassword);
    % dbs-Following from http://www.mathworks.com/support/solutions/en/data/1-3PRRDV/
    ...
    try
        ...
        % Better isolation of error sometime
        result = 'unspecified text message error';
    end
end
