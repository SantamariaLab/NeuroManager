...
% sendEmail
% Sends an email to the user with the specified subject and message and
% attachment set. 
% Part of the NotificationSet class.
% For reference see the text message function as well as
% http://www.mathworks.com/support/solutions/en/data/1-CZXO98/?solution=1-CZXO98
function result = sendEmail(obj, subject, message, attachment)
    ...
    try
        if ~isempty(attachment)
            sendmail(obj.notifications.authEmail, subject, message, attachment);
        else
            sendmail(obj.notifications.authEmail, subject, message);
        end
        result = '';
    catch
        % Better isolation of error sometime
        result = 'unspecified sendmail error';
    end
end
