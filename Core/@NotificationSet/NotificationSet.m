...
% NotificationSet
% A class that provides facilities for notifying the user of events via
% TEXT (SMS, or MMS, if there is an attachment)
% or email or both.
% Type: 'EMAIL' or 'TEXT' or 'BOTH'
% Address: '10 digit phone number' (no hyphen)
% Carrier: % see (UNK) functions for choices
% SMTP_Server: for EMAIL and for TEXT authentication
% AuthEmail: for TEXT and EMAIL
% AuthPasswd: for TEXT and EMAIL
classdef NotificationSet < handle
    properties
        % Holds user data for the communications
        notifications = struct('inUse',         false,...
                               'enabled',       false,...
                               'type',          '',...
                               'address',       '',...
                               'carrier',       '',...
                               ...
                               'smtpServer',   '',...
                               'smtpPassword', '');  
        % Number of seconds pause after sending a notification
        % to help make delivery more reliable.
        messageDelay = 2.0;
        
        % For logging the notifications sent
        simLogHandle;   
    end
    
    methods
        function obj = NotificationSet()
                obj.notifications.inUse = false;
                obj.notifications.enabled = false;
        end 
        
        function setup(obj, type, address, carrier,...
                                            authEmail, smtpServer, delay)
        % Queries the user for password and sets up the data struct
        % Later do an input parser on this
            if (strcmp(type, 'EMAIL') ||...
                strcmp(type, 'TEXT') ||...
                strcmp(type, 'BOTH'))
                % Get the authentification password from the user
                % Note: this is non-secure handling, which seemed
                % appropriate for now. Failure of the password, for now, is
                % indicated by the user not getting the "RunSet
                % Started" notification.
                dialogTitle = 'Notification Password Request';
                beep;
                if usejava('Desktop')
                    prompt = ['Please enter the SMTP Validation Password for '...
                              address ' on ' carrier '.'];
                    inputPwd = inputdlg(prompt, dialogTitle); 
                    obj.notifications.smtpPassword = inputPwd{1};
                else
                    prompt = ['Please enter the SMTP Validation Password for '...
                           address ' on ' carrier ': '];
                    obj.notifications.smtpPassword = input(prompt, 's');
                end

                obj.notifications.type = type;
                obj.notifications.address = address;
                obj.notifications.carrier = carrier;
                obj.notifications.authEmail = authEmail;
                obj.notifications.smtpServer = smtpServer;
                obj.notifications.inUse = true;
                obj.notifications.enabled = true;
                obj.messageDelay = delay;
            else
                error(['NotificationSet:Setup', 'Incorrect Type received: '...
                       type '.']);
            end    
        end
        
        function setLog(obj, simlog)
                obj.simLogHandle = simlog;
        end
        ...
        function type = getType(obj)
            type = obj.notifications.type;
        end
        
        function result = inUse(obj)
            result = obj.notifications.inUse;
        end
        
        function result = isEnabled(obj)
            result = obj.notifications.enabled;
        end
        
        % ----------------
        % Note: uses the log, so must be used after log is set up
        function send(obj, sbj, msg, attch)
        % Sends the input notification message with the input subject
            if (strcmp(obj.notifications.type, 'EMAIL')|| ...
                strcmp(obj.notifications.type, 'BOTH'))
                result = obj.sendEmail(sbj, msg, attch);
                if isempty(result)
                    obj.simLogHandle.write(['EMAIL Notification sent: <' ...
                                    sbj '> <' msg '> ' ...
                                    obj.notifications.authEmail ' ' ...
                                    obj.notifications.smtpServer ' ' ...
                                    '<not logging password>']);
                else
                    obj.simLogHandle.write(['EMAIL Notification failed: <' ...
                                            'Intended message was: '...
                                            sbj '> <' msg '> ']);
                end
                    
            end
            if (strcmp(obj.notifications.type, 'TEXT')|| ...
                strcmp(obj.notifications.type, 'BOTH'))
                result = obj.sendTextMsg(sbj, msg, attch);
                if isempty(result)
                    obj.simLogHandle.write(['TEXT Notification sent: ' ...
                                obj.notifications.address ' ' ...
                                obj.notifications.carrier ' <' ...
                                sbj '> <' msg '> ' ...
                                obj.notifications.authEmail ' ' ...
                                obj.notifications.smtpServer ' ' ...
                                '<not logging password>']);
                else
                    obj.simLogHandle.write(['TEXT Notification failed: ' ...
                                        'Intended message was: <'...
                                        sbj '> <' msg '> ']);
                end
            end
            pause(obj.messageDelay);
        end
    end
end
