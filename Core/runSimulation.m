...
% runSimulation.m 
% The interface between NeuroManager and the user code ON THE TARGET. Calls
% the user-defined MATLAB function UserSimulation, which will be compiled 
% together with RunSimulation. The user DOES NOT EDIT RunSimulation.m; the
% user CREATES UserSimulation.m with the interface format seen here.
%
% RunSimulation tells the user where to find input files, where to put
% output data, where to run the code, what the simulation and machine ids
% are, and what the input parameters are for the simulation with that id.
% Collects time data, creates a report, zips up the output for download,
% and signals the host that it's done.
function result = runSimulation(machineID, simID, runDir, ... 
                                inDir, modelDir, outDir,...
                                varargin) 
    % Tell the simulator the simulation is running
    f = fopen(fullfile(outDir, 'RUNNING'), 'w');
    ...
    tic;
    [result, errMsg] =...
        userSimulation(machineID, simID, runDir, inDir, modelDir, outDir,...
                       varargin{1:length(varargin)});
	simTime = toc;

    % Convert result to what the NeuroManager code uses 
    if result == 0
        resultStr = 'COMPLETE';
    else
        resultStr = 'FAILED';
    ...
    % This file gives the details of the results.
    saveResults(simID, outDir, resultStr, simTime, errMsg);
...
% This used on the remote to produce the results file.
function saveResults(simID, outDir, resultStr, simTime, errMsg)
    f = fopen(fullfile(outDir, 'SimulationResults.txt'), 'w');
    if isempty(errMsg)
        errMsg = '<no message>';
    end
    fprintf(f, 'ID %s\n', simID);
    fprintf(f, 'Result %s\n', resultStr);
    fprintf(f, 'Simtime %10.2f\n', simTime);
    fprintf(f, 'Errmsg %s\n', errMsg);
