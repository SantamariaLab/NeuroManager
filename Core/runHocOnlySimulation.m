...
% Strings together some target-side utilities to run a Hoc-only Neuron
% simulation. Easily modified for other approaches.
%
...
function result = runHocOnlySimulation(runDir, inputDir,...
    ...
    nrnivShell = createHocOnlyNrnivsh(md, runDir, inputDir, outputDir, hocFile);
    ...
    % from the run dir.
    sysCommand =...
        ['cd ' runDir '; chmod +x ' nrnivShell '; ./' nrnivShell '; '];
