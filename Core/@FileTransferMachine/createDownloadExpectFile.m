...
% Part of the FileTransferMachine class.
% Creates an expect file for asynchronous download of simulation results
% when dual key authorization is not in play and the host is UNIX. Called
% by fileFromMachineNoWait().
% Can be overridden in subclasses for localization.
function createDownloadExpectFile(obj, hostdir, targetpath)
    ...
    f = fopen(fullfile(hostdir, 'Download.exp'), 'w');
    % path conversion not necessary - we know it's UNIX
    % (not Windows)
    fprintf(f, '%s\n', '#!/usr/bin/expect -f'); 
    fprintf(f, '%s\n',...
        ['spawn scp ' obj.fsUserName...
         '@' obj.fsIPAddress ':'...
         path2UNIX(targetpath) ' '...
         path2UNIX(fullfile(hostdir, [tfilename tfileext]))...
         ]);
    % Uncomment following line if detail desired but will
    % leave password in dwnldstderr.txt
    %fprintf(f, '%s\n', 'exp_internal 1'); 
    fprintf(f, '%s\n', 'set timeout -1');
