...
% fileFromMachineNoWait.m
% Part of the FileTransferMachine class definition.
% Initiates parallel transfer of a file from target to host.
% Highly host-type dependent.
% hostdir is the directory to which the file will be copied
% with the same filename. Creates signalfilename or errorfilename
% (whichever is appropriate) on the host in hostdir when download
% is complete. 
% ----------------
function fileFromMachineNoWait(obj, hostDir, targetPath,...
                               signalFilename, errorFilename)
    % HOST = TARGET (LINUX)
    if strcmp(obj.hostID, obj.id)
        % Assume this is fast enough not to matter that it's not in
        % the background.
        status = copyfile(targetPath, hostDir);
        if status==0
            error('NeuroManager:FileTransferMachine', 'copyfile failed.');
        end
        % Already done so signal it
        [f, ~] = fopen(fullfile(hostDir, signalFilename), 'w');
        fclose(f);
    else
        % WINDOWS HOST
        % Signal files currently not guaranteed to appear after
        % download appears.
        if (obj.hostOS == OSType.WINDOWS)
            % Create a batch file that ensures the download is complete
            % before the signal file appears, then run it in the
            % background.
            f = fopen(fullfile(hostDir, 'Download.bat'), 'w');
            fprintf(f, '%s\n', ['setlocal']);
            fprintf(f, '%s\n', ['cd ' hostDir]);
            % For cloud hosts we have to add in the hostkey blob to
            % keep PuTTY from objecting that it hasn't seen the server
            % before. The matlab ssh library doesn't check the host key
            % (bad), but putty does (good). See
            % http://www.chiark.greenend.org.uk/~sgtatham/putty/wishlist/accept-host-keys.html
            if ~isempty(obj.hostKeyFingerprint)
                hostKeyOption = [' -hostkey ' obj.hostKeyFingerprint ' '];
            else
                hostKeyOption = '';
            end
            % The download command
            if obj.fsUseDualKey
                xferCommand = ['pscp -batch  -2 '...
                               hostKeyOption ...
                               ' -l ' obj.fsUserName...
                               ' ' obj.fsIPAddress...
                               ':'  path2UNIX(targetPath) ' '...
                               hostDir ...
                               ' 1> dwnldstdout.txt 2> dwnldstderr.txt']
            else
                xferCommand = ['pscp -batch -l '... 
                               hostKeyOption ...
                               obj.fsUserName ' -pw ' obj.fsPassword...
                               ' ' obj.fsIPAddress ':' ...
                               path2UNIX(targetPath) ' ' hostDir ...
                               ' 1> dwnldstdout.txt 2> dwnldstderr.txt']
            end
            fprintf(f, '%s\n', xferCommand);
            fprintf(f, '%s\n', ['if %ERRORLEVEL% EQU 0 (']);
            fprintf(f, '%s\n', ['   type nul>>' signalFilename]);
            fprintf(f, '%s\n', ['                ) else (']);
            %fprintf(f, '%s\n', ['else (']);
            ...
            % LINUX HOST
            if obj.fsUseDualKey
                [~, tfilename, tfileext] = fileparts(targetPath);
                % Make a shell file that runs the download then creates the
                % signal file
                f = fopen(fullfile(hostDir, 'Download.sh'), 'w');
                fprintf(f, '%s\n', ['#!/bin/bash']);
                fprintf(f, '%s\n', ['cd ' hostDir]);
                fprintf(f, '%s\n', ['source ' obj.getAgentInfo()]);
                fprintf(f, '%s\n', ...
                    ['scp -B ' obj.fsUserName...
                     '@' obj.fsIPAddress ':'...
                     path2UNIX(targetPath) ' '...
                     path2UNIX(fullfile(hostDir, [tfilename tfileext]))...
                     ' 1>dwnldstdout.txt 2>dwnldstderr.txt'...
                     ]);
                fprintf(f, '%s\n', ['result=$?']);
                fprintf(f, '%s\n', ['if [ $result -eq 0 ]']);
                fprintf(f, '%s\n', ['then']);
                fprintf(f, '%s\n', ['   touch ' fullfile(hostDir, signalFilename)]);
                fprintf(f, '%s\n', ['else']);
                fprintf(f, '%s\n', ['   touch ' fullfile(hostDir, errorFilename)]);
                fprintf(f, '%s\n', ['fi']);
                fclose(f);
                system(['cd ' hostDir '; chmod +x Download.sh']);
                % Do the shell file in the background
                system(['cd ' hostDir '; ./Download.sh &']);
            else
                % Not the best way (to use a password), but
                % sometimes dual key is not available.
                % Make a download expect file that sends the password too
                % Note: expect file will contain the password
                obj.createDownloadExpectFile(hostDir, targetPath);
                % Make a shell file that runs the download then creates the
                % signal file
                f = fopen(fullfile(hostDir, 'Download.sh'), 'w');
                fprintf(f, '%s\n', '#!/bin/bash');
                fprintf(f, '%s\n', 'expect Download.exp 1>dwnldstdout.txt 2>dwnldstderr.txt');
                fprintf(f, '%s\n', ['result=$?']);
                fprintf(f, '%s\n', ['if [ $result -eq 0 ]']);
                fprintf(f, '%s\n', ['then']);
                fprintf(f, '%s\n', ['   touch ' fullfile(hostDir, signalFilename)]);
