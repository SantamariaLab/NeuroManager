...
% NeuronMachine
% A class that provides Neuron-associated machine locations and other
% services for its subclasses. 
classdef NeuronMachine < handle
    properties
        neuronDir;          % Full path of the Neuron software, if any
        neuronBinExt;       % Full path of the Neuron bin directory
        neuronLibExt;       % Full path of the Neuron lib directory
        neuronHomeExt;      % Full path of the Neuron share directory
        neuronPythonLibExt; % Full path of the Neuron python library extension
        pythonPath;         % Full path of Python
    end
    
    methods (Abstract)
        % NeuronMachines have to say where model files will be compiled
        getCompileNeuronModelFilesStrPhaseP(obj)
        getCompileNeuronModelFilesStrPhaseD(obj)
    end
    
    methods
        function obj = NeuronMachine(md)
            obj.neuronDir = md.getSetting('neuronDir');
            obj.neuronBinExt = md.getSetting('neuronBinExt');
            obj.neuronLibExt = md.getSetting('neuronLibExt');
            obj.neuronHomeExt = md.getSetting('neuronHomeExt');
            obj.neuronPythonLibExt = md.getSetting('neuronPythonLibExt');
            obj.pythonPath = md.getSetting('pythonPath');
        end
        ...
        function path = getNeuronBinPath(obj)
            path = fullfile(obj.neuronDir, obj.neuronBinExt);
        end
        
        % ----------
        function path = getNeuronLibPath(obj)
            path = fullfile(obj.neuronDir, obj.neuronLibExt);
        end
        
        % ----------
        function path = getNeuronHomePath(obj)
            path = fullfile(obj.neuronDir, obj.neuronHomeExt);
        end

        % ----------
        function path = getNeuronPythonLibPath(obj)
            path = fullfile(obj.neuronDir, obj.neuronPythonLibExt);
        end
